...
namespace DarkSkyDotNetCore
{
    public class ForecastRequest
    {
        ...
        private const string CurrentForecastUrl = "https://api.darksky.net/forecast/{0}/{1},{2}?units={3}&extend={4}&exclude={5}&lang={6}";
        private const string PeriodForecastUrl = "https://api.darksky.net/forecast/{0}/{1},{2},{3}?units={4}&extend={5}&exclude={6}&lang={7}";

        public ForecastRequest(string apiKey, float latF, float longF, Unit unit, string lang = "", Extend[] extend = null, Exclude[] exclude = null)
        {
            _apiKey = apiKey;
        ...
        public ForecastRequest(string apiKey, float latF, float longF, DateTime time, Unit unit, string lang = "", Extend[] extend = null, Exclude[] exclude = null)
        {
            _apiKey = apiKey;
            _latitude = latF.ToString(CultureInfo.InvariantCulture);
            _longitude = longF.ToString(CultureInfo.InvariantCulture);
        ...
        {
            var url = (_time == null) ? String.Format(CurrentForecastUrl, _apiKey, _latitude, _longitude, _unit, _extend, _exclude, _lang) :
                String.Format(PeriodForecastUrl, _apiKey, _latitude, _longitude, _time, _unit, _extend, _exclude, _lang);
